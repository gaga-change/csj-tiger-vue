  ...
  <div class="menu-wrapper">
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      <template v-if="item.outLinkUrl">
        <a v-if="item.children.length===1 && !item.children[0].children && !item.alwaysShow" target="_blank" :href="item.outLinkUrl + enterprise"
        ...
        <el-submenu v-else :index="item.name||item.path" :key="item.name">
          ...
          <template v-for="child in item.children" v-if="!child.hidden">
            ...
            <a v-else target="_blank" :href="child.outLinkUrl + enterprise" :key="child.name">
