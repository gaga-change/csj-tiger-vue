        ...
        <div style="margin:8px 0 12px">
          ...
          >
            ...
            { required: true, message: '该项为必填'},
                ...
                <span
                  v-show="filesRequired"
                  style="color:#f56c6c;font-size:12px;margin-left:70px;top:132px;left:0;position: absolute;"
                > 附件为必选</span>
              </el-col>
...
export default {
  ...
  methods: {
    ...
    handleEnclosureUploadSuccess(res, file, fileList) {
      ...
      this.fileList = fileList.filter(v => !(v.response && v.response.code !== '200'))
      if (res.code === '200') {
      } else {
        this.$message({
          message: res.message || `图片【${file.name}】上传异常，稍后重试或联系管理员！`,
          type: 'error'
        })
      }
    },
    submitEnclosureUpload() {
      if (this.fileList.length === this.$refs.filesupload.uploadFiles.length) {
        return this.$message('请选取文件！')
      }
      this.readyUploadNum = this.$refs.filesupload.uploadFiles.length - this.fileList.length
